...
export default {
  ...
  computed: {
    student () {
      return this.$store.getters.findStudent();
    },
    ...mapGetters(['isLoaded', 'findStudent'])
  },
  methods: {
    async submit() {
      const firstName = this.firstName || this.findStudent(this.$route.params.id).firstName;
      const lastName = this.lastName || this.findStudent(this.$route.params.id).lastName;
